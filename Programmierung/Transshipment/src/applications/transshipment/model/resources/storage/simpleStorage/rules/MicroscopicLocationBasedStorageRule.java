...
import applications.transshipment.model.resources.storage.simpleStorage.SimpleStorageRow.SimpleSubStorageRow;
 ...
 * @author bode
 */
public class MicroscopicLocationBasedStorageRule implements StorageRule<LocationBasedStorage> {

    private final LocationBasedStorage resource;

    public MicroscopicLocationBasedStorageRule(LocationBasedStorage s, InstanceHandler handler) {
        this.resource = s;
    }
...
//    }
    ...
    public List<StoreOperation> getPossibleStoreOperations(LoadUnit loadUnit, Schedule s, FieldElement start, FieldElement duration) {
        ...
        for (SimpleStorageRow row : this.resource.getRows()) {
             ...
             */
                ...
                SimpleSubStorageRow sdssr = new SimpleSubStorageRow(set, row);
                //... und fuege dessen PosOps hinzu.
                StorageRule rule = (StorageRule) s.getHandler().get(sdssr);
                result.addAll(rule.getPossibleStoreOperations(loadUnit, s, start, duration));
                while (!tmp.isEmpty()) {
                    set.add(tmp.remove(0));
                    set.remove(0);
                    sdssr = new SimpleSubStorageRow(set, row);
                    StorageRule rule2 = (StorageRule) s.getHandler().get(sdssr);
    ...
    public void schedule(Operation o, Schedule s, FieldElement start) {
        /**
         * Wird über die SubOperationen abgefertigt.
         */
//        StoreOperation storeOp = (StoreOperation) o;
//        for (StorageLocation storageLocation : resource.getStorageLocations()) {
//            DefaultStoreOperation ssOp = new DefaultStoreOperation(storeOp.getLoadUnit(), storageLocation, storeOp.getDuration());
