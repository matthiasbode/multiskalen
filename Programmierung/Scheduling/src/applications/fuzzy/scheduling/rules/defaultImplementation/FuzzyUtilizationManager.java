...
import applications.fuzzy.demo.Validation;
import static applications.fuzzy.demo.Validation.dx;
 ...
 * @author bode
 */
public class FuzzyUtilizationManager implements UtilizationManager {
    ...
    public FuzzyUtilizationManager(Resource r, double capacity, TimeSlot t) {
    ...
    public FuzzyUtilizationManager(Resource r, double capacity, FieldElement start) {
        this.capacity = new DoubleValue(capacity);
        this.r = r;
    ...
    public FuzzyUtilizationManager(Resource r, double capacity, FieldElement start, boolean denyStartAdaption) {
        this.capacity = new DoubleValue(capacity);
        this.r = r;
        this.t = new TimeSlot(FuzzyFactory.createCrispValue(start.doubleValue()), FuzzyFactory.createCrispValue(r.getTemporalAvailability().getUntilWhen().doubleValue()));
        this.workloadFuction = new LinearizedFunction1d();
    ...
    private boolean isScheduableInBestCase(FuzzyInterval adaptedStart, BetaOperation o, LinearizedFunction1d currentWorkloadFunction) {
        ...
        if (demand != null) {
            ...
            if (ParallelTools.PLOT) {
                System.out.println("TEST!");
                FuzzyFunctionPlotter plotterForOp = new FuzzyFunctionPlotter("Gesamtauslastung der Ressource '");
                plotterForOp.addFunction(result, result.getValues().higherKey(result.getValues().firstKey()), result.getValues().lowerKey(result.getValues().lastKey()), dx, "Auslastung");
                plotterForOp.addFunction(adaptedStart.membership, result.getValues().higherKey(result.getValues().firstKey()), result.getValues().lowerKey(result.getValues().lastKey()), dx, "Start");
                plotterForOp.addFunction(FuzzyDemandUtilities.getNecessityFunction(o, adaptedStart), result.getValues().higherKey(result.getValues().firstKey()), result.getValues().lowerKey(result.getValues().lastKey()), dx, "Notwendigkeit");
                plotterForOp.addFunction(FuzzyDemandUtilities.getPossibilityFunction(o, adaptedStart), result.getValues().higherKey(result.getValues().firstKey()), result.getValues().lowerKey(result.getValues().lastKey()), dx, "Möglichkeit");
                System.out.println("TEST!");
                plotterForOp.plot();
                ParallelTools.PLOT = false;

            }
            Double maxValue = result.getMax().getValue();
            if (maxValue <= capacity.doubleValue() + 0.0001) {
                return true;
            }
    ...
    private boolean testTranslateBecauseOfCapacity(FuzzyWorkloadParameters lambda, FuzzyInterval adaptedStart, BetaOperation o, LinearizedFunction1d currentWorkloadFunction) {
        LinearizedFunction1d demand = FuzzyDemandUtilities.getDemandFunctionAtPessimisticLevelOfResourceWithLambda(o, r, (FuzzyInterval) adaptedStart, lambda);
        if (demand != null) {
            LinearizedFunction1d result = currentWorkloadFunction.add(demand);
            LinearizedFunction1d t = new LinearizedFunction1d(0., (double) System.currentTimeMillis(), 1.0);
            LinearizedFunction1d tmp = result.sub(t);
            tmp.eliminateNotRequiredValues();
            ArrayList<Double> nullstellen = tmp.getSupportValues(0.001);
            if (nullstellen.isEmpty()) {
                return false;
            }
            nullstellen.remove(Double.NEGATIVE_INFINITY);
            nullstellen.remove(Double.POSITIVE_INFINITY);
            Double posViolation = nullstellen.get(0);
            if (adaptedStart.getM1() > posViolation) {
                return true;
            }
        }
        return false;
...
//        while (adaptedStart != null) {
        ...
        double lambdaMin = FuzzyDemandUtilities.getLambdaMin(o, adaptedStart);
        double lambdaMax = FuzzyDemandUtilities.getLambdaMax(o, adaptedStart);

        /**
         * Falls möglich, dann mit großem Lambda testen.
         */
        if (test) {
//            double lambdaTest = lambdaMin + o.getBeta() * (lambdaMax - lambdaMin);
//            FuzzyWorkloadParameters lambda = new FuzzyWorkloadParameters(lambdaTest);
...
//            }
            ...
            lambda.adaptedStart = adaptedStart;
            while (lambda.lambdaL > 0.0) {
                boolean testCapacity = testCapacity(lambda, adaptedStart, o, currentWorkloadFunction);
                if (testCapacity) {
                    s.fuzzyWorkloadParameters.put(o, lambda);
                    return true;
                } else {
//                    boolean isViolatingBeforeM1 = testTranslateBecauseOfCapacity(lambda, adaptedStart, o, currentWorkloadFunction);
//                    if (isViolatingBeforeM1) {
//                        return false;
//                    }
                }
                /**
                ...
                if (denyStartAdaption) {
                    ...
                    lambda.lambdaL = lambda.lambdaL - 0.01;
                }
            }
        }
...
//        start= lambda.adaptedStart;
        if (start instanceof FuzzyNumber) {
            ...
            if (f == null) {
//                FuzzyDemandUtilities.getDemandFunctionAtPessimisticLevelOfResourceWithLambda(o, r, (FuzzyInterval) start, lambda);
