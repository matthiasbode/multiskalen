...
public class Canvas2D extends JComponent {
    ...
    public static final int MOUSE_MOVE = 138;
    public static final int MOUSE_ZOOM = 139;
    ...
    private MouseListener_MOVE mouselistener_MOVE;
     ...
     */
    public void activateMouseControl(int type, boolean mode) {
        if (type == MOUSE_MOVE) {
            if (mode) {
                removeMouseListener(mouseListener_ZOOM01);
                if (mouselistener_MOVE == null) {
                    mouselistener_MOVE = new MouseListener_MOVE();
                }

                addMouseListener(mouselistener_MOVE);
            } else {
                removeMouseListener(mouselistener_MOVE);
            }
        } else if (type == MOUSE_ZOOM) {
            if (mode) {
                removeMouseListener(mouselistener_MOVE);
    ...
    public int getMouseMode() {
        ...
        if (m.equals(mouselistener_MOVE)) {
            return MOUSE_MOVE;
        } else if (m.equals(mouseListener_ZOOM01)) {
            return MOUSE_ZOOM;
        } else {
            return -1;
        }
...
//                    	if(!intersects(s))throw new IllegalArgumentException("Shape not intersects Link");
    ...
    public static void main(String[] args) {
        ...
        canvas.activateMouseControl(Canvas2D.MOUSE_ZOOM, true);
